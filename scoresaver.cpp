...
bool ScoreSaver::SaveScore(QString fileName, QVector<QString>* data,QString comments, QString folder,QString compas, int lastErrors,int speed, int subdivisions){
    ...
    SaveXML("D:\\GitHub\\MUSVI_Xuzon\\NuevaPrueba.xml",json);
    ...
    l_mXml.appendChild(l_root);
        ...
        l_root.appendChild(l_partList);
            ...
            l_partList.appendChild(l_scorePart);
                ...
                QDomText l_textName=l_mXml.createTextNode(f_json["name"].toString());
            ...
            l_part.appendChild(l_measure1);
                ...
                l_measure1.appendChild(l_attributes);
                    ...
                    l_attributes.appendChild(l_time);
                        ...
                        QDomElement l_beats=l_mXml.createElement("beats");
                        QDomText l_numberBeats=l_mXml.createTextNode(l_compas.at(0));
                        l_beats.appendChild(l_numberBeats);
                        l_time.appendChild(l_beats);
                        QDomElement l_beattype=l_mXml.createElement("beat-type");
                        QDomText l_numberBeattype=l_mXml.createTextNode(l_compas.at(2));
                        l_beattype.appendChild(l_numberBeattype);
                        l_time.appendChild(l_beattype);


                    QDomElement l_clef=l_mXml.createElement("clef");
                    l_attributes.appendChild(l_clef);
                ...
                int l_dataSize=l_dataMeasure.size();
                int l_compasNumber=l_compas[0].digitValue();
                qDebug()<<"SaveXML="<<l_compasNumber;
                QVector<XMLNote> l_notesOfPulse;
                for(int i=0; i<l_dataSize;i++)
                {
                    int l_compasCount=0;
                    l_notesOfPulse=PulseToXMLNotes(l_dataMeasure.at(i));
